document.addEventListener('DOMContentLoaded', () => {
    ...
    function moveLeft() {
        ...
        if(current.some(index => squares[currentPosition + index].classList
            .contains('taken'))) {
            currentPosition +=1
        }
        draw()
    ...
    function moveRight() {
        undraw()
        const isAtRightEdge = current.some(index => (currentPosition + index) % 
        width === width -1)
    ...
    function displayShape() {
        ...
        upNextTetrominoes[nextRandom].forEach( index => {
            ...
            displaySquares[displayIndex + index]
    ...
    function addScore() {
        for (let i = 0; i < 199; i +=width) {
            ...
            if(row.every(index => squares[index].classList
                .contains('taken'))) {
                score +=10
